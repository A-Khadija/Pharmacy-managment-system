    ...
    public partial class newprod : Form
    {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("etes vous sur de supprimmer ce medicament?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                cmd = new SqlCommand("suppProduit", conn);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@Produitid", SqlDbType.VarChar).Value = textBox1.Text;
                cmd.ExecuteNonQuery();
        ...
        {
            conn.Open();
            SqlCommand cd = new SqlCommand("insert  into Produit (IDProduit,designation,forme,prixUn,dateExp,status,codeBarre,Categorie) values(@Produitid,@designation,@forme,@prixUn,@dateExp,@status,@codeBarre,@Categorie) ;", conn);
        ...
        {
            conn.Open();
            SqlCommand cd = new SqlCommand("update Produit set designation=@designation,forme=@forme,prixUn=@prixUn,dateExp=@dateExp,status=@status,codeBarre=@codeBarre,Categorie=@Categorie where IDproduit=@Produitid ;",conn);
            cd.Parameters.AddWithValue("@Produitid",textBox1.Text);
            cd.Parameters.AddWithValue("@designation",textBox2.Text);
            cd.Parameters.AddWithValue("@forme",comboBox3.Text);
            cd.Parameters.AddWithValue("@prixUn",Decimal.Parse(textBox3.Text));
           ...
           /* cmd.Parameters.AddWithValue("@forme", SqlDbType.VarChar).Value = comboBox3.Text;
            cmd.Parameters.AddWithValue("@prixUn", SqlDbType.VarChar).Value = textBox3.Text;
            cmd.Parameters.AddWithValue("@dateExp", SqlDbType.VarChar).Value = dateTimePicker1.Text;
            cmd.Parameters.AddWithValue("@status", SqlDbType.VarChar).Value = comboBox2.Text;
            cmd.Parameters.AddWithValue("@codeBarre", SqlDbType.VarChar).Value = textBox5.Text;
            cmd.Parameters.AddWithValue("@Categorie", SqlDbType.VarChar).Value = comboBoxcatego.Text;
            cmd.Parameters.AddWithValue("@Qte_stck", SqlDbType.VarChar).Value = textBox6.Text;
            cmd.Parameters.AddWithValue("@pph", SqlDbType.VarChar).Value = textBox8.Text;
            cmd.Parameters.AddWithValue("@ppv", SqlDbType.VarChar).Value = textBox7.Text;
            cmd.Parameters.AddWithValue("@QteDisp", SqlDbType.VarChar).Value = textBox4.Text;*/
